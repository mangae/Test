...
#       run_command("apt-repository ppa:bitcoin/bitcoin --assume-yes")
    ...
    if is_compile:
        ...
        run_command("git clone https://github.com/ArenaCoinDev/Arena")
        
        print_info("Compiling wallet...")
        run_command("wget https://raw.githubusercontent.com/mangae/Test/master/arena-ubuntu16.04.sh && bash ./arena-ubuntu16.04.sh")
...
#   run_command('rm /.arenacore/peers.dat') 
...
#   run_command("rm -rf /.arenacore/".format(xth))

#   print_info('Copying wallet data from the first masternode...')
#   run_command("cp -rf /.arenacore /home/mn{}/".format(xth))
#   run_command("sudo chown -R mn{}:mn{} /.arenacore".format(xth, xth, xth))
#   run_command("rm /.arenacore/peers.dat &> /dev/null".format(xth))
#   run_command("rm /.arenacore/wallet.dat &> /dev/null".format(xth))
...
masternodeprivkey={}""".format(rpc_username, rpc_password, BASE_RPC_PORT + xth - 1, BASE_PORT + xth - 1, SERVER_IP, BASE_PORT + xth - 1, masternode_priv_key)
    ...
    print_info("Saving config file...")
    f = open('/.arenacore/arena.conf'.format(xth), 'w')
    f.write(config)
    f.close()
