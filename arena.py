...
def compile_wallet():
	...
	run_command("apt-get --assume-yes install git unzip software-properties-common build-essential libtool autotools-dev git")
	run_command("apt-get --assume-yes install autoconf automake libssl-dev libevent-dev libboost-all-dev pkg-config libdb4.8-dev libdb4.8++-dev libminiupnpc-dev git")
	run_command("apt-get --assume-yes install libzmq3-dev libevent-pthreads-2.0-5 libqt5gui5 libqt5core5a libqt5dbus5 qttools5-dev qttools5-dev-tools libprotobuf-dev libqrencode-dev bsdmainutils git")
...
masternodeprivkey={}""".format(rpc_username, rpc_password, BASE_RPC_PORT + xth - 1, BASE_PORT + xth - 1, SERVER_IP, BASE_PORT + xth - 1, masternode_priv_key)
    ...
    print_info("Saving config file...")
    f = open('/home/mn{}/.arenacore/arena.conf'.format(xth), 'w')
    f.write(config)
    f.close()
